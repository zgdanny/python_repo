...
class Enemy(pygame.sprite.Sprite):
    ...
    def load_frames(self, frame_rects):
        for frame_rect in frame_rects:
            left_frame = tools.get_image(
                setup.GRAPHICS['enemies'], *frame_rect, (0, 0, 0), constants.ENEMY_MULTI)
            right_frame = pygame.transform.flip(
                left_frame, True, False)  # 第一个参数图像，第二个是否水平翻转，第三个是否垂直翻转
    ...
    def check_y_collisions(self, level):
        ...
        check_group = pygame.sprite.Group(
            level.ground_items_group, level.brick_group, level.box_group)
