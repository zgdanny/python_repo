...
class Box(pygame.sprite.Sprite):
    def __init__(self, x, y, box_type, group, name='box', color=None):
        ...
        for frame_rect in self.frame_rects:
            self.frames.append(tools.get_image(
                setup.GRAPHICS['tile_set'], *frame_rect, (0, 0, 0), constants.BRICK_MULTI))
    ...
    def bumped(self, level):
        ...
        if self.rect.y > self.y+5:
            ...
            if self.box_type == 1:
                self.group.add(Creat_coin(
                    self.rect.centerx, self.rect.centery))
                self.group.add(Text_info(self.rect.centerx,
                               self.rect.centery, '+ 200'))
            elif self.box_type == 3:
                if level.player.big:
                    self.group.add(create_powerup(
                        self.rect.centerx, self.rect.centery, self.box_type, 1))
                else:
                    self.group.add(create_powerup(
                        self.rect.centerx, self.rect.centery, self.box_type, 0))
                print(self.group)
            elif self.box_type == 4:
                self.group.add(create_powerup(self.rect.centerx,
                               self.rect.centery, self.box_type, 3))
            elif self.box_type == 5:
                self.group.add(create_powerup(self.rect.centerx,
                               self.rect.centery, self.box_type, 2))
...
class Creat_coin(pygame.sprite.Sprite):
    def __init__(self, centerx, centery):
        pygame.sprite.Sprite.__init__(self)
        self.name = 'coin'
        frame_rects = [(0, 112, 16, 16), (16, 112, 16, 16),
                       (32, 112, 16, 16), (48, 112, 16, 16)]
        self.frames = []
        self.frame_index = 0
